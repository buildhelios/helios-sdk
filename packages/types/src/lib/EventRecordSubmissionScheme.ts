/* <ALLOW_AUTO_DELETE DEPENDENCIES="EventRecordSubmission" /> */
// this file was auto generated by @iyio/protogen - https://github.com/iyioio/common/tree/main/packages/protogen
import { z } from 'zod';
import { CommsChannelScheme } from './CommsChannelScheme';

const __base__EventRecordSubmissionScheme=z.object({
    profileAltId:z.string().max(255).optional(),
    id:z.number().int(),
    profileId:z.number().int().optional().describe("Id of the user profile that triggered the event"),
    cdi:z.string().max(255).optional().describe("Client device id - Client generated uuid"),
    sid:z.string().max(255).optional().describe("Session Id - Client generated uuid"),
    productId:z.number().int().optional().describe("Id of a product associated with the event"),
    productAltId:z.string().max(255).optional().describe("Alternate product id"),
    locationId:z.number().int().optional().describe("Id of a location associated with the event"),
    locationAltId:z.string().max(255).optional().describe("Alternate location id"),
    accountId:z.number().int().optional().describe("Id of the account associated with the event"),
    messageId:z.number().int().optional(),
    type:z.string().max(255).describe("The events type"),
    time:z.number().describe("A timestamp of when the event occurred"),
    data:z.record(z.any()).optional(),
    importSource:z.string().max(255).optional().describe("Custom event data"),
    longText:z.string().optional().describe("A large unindexed string associated with the event"),
    currencyValue:z.number().optional(),
    text:z.string().max(255).optional().describe("A short indexed string associated with the event"),
    tags:z.string().max(255).array().optional().describe("Array of tags that can be used to group events"),
    path:z.string().max(255).optional().describe("URL path where the event occurred"),
    host:z.string().max(255).optional().describe("Host / domain where the event occurred"),
    elem:z.string().max(255).optional().describe("Name of element associated with the event. Mostly used with elemView events"),
    classList:z.string().max(255).array().optional().describe("List of classes of the element target triggered the event"),
    x:z.number().optional().describe("Browser page x"),
    y:z.number().optional().describe("Browser page y"),
    sx:z.number().optional().describe("Browser scroll x"),
    sy:z.number().optional().describe("Browser scroll y"),
    num:z.number().optional(),
    str:z.string().max(255).optional(),
    bool:z.boolean().optional(),
    int:z.number().int().optional(),
    utmCampaign:z.string().max(255).optional().describe("Campaign name. example - summer-sales or fall-discounts"),
    utmSource:z.string().max(255).optional().describe("Where traffic is coming from for example - google or facebook"),
    utmMedium:z.string().max(255).optional().describe("What type of medium traffic is coming from.\nexample - sms, email, social, search"),
    utmContent:z.string().max(255).optional().describe("What type of content was interacted with.\nexample - image, nav-link, menu-link"),
    utmTerm:z.string().max(255).optional().describe("The copy the generated traffic. This will often be the tag line of an ad."),
    ip:z.string().max(255).optional().describe("IP address the event came from"),
    lat:z.number().optional(),
    lng:z.number().optional(),
    postalCode:z.string().max(255).optional(),
    city:z.string().max(255).optional(),
    region:z.string().max(255).optional(),
    state:z.string().max(255).optional(),
    country:z.string().max(255).optional(),
    formId:z.number().int().optional(),
    submissionId:z.number().int().optional(),
    automationId:z.number().int().optional(),
    automationNodeId:z.string().max(255).optional(),
    targetUrl:z.string().optional(),
});
const __lazy__EventRecordSubmissionScheme=z.object({
    channel:z.lazy(()=>CommsChannelScheme).optional(),
});
export const EventRecordSubmissionScheme:(typeof __base__EventRecordSubmissionScheme)=__base__EventRecordSubmissionScheme.merge(__lazy__EventRecordSubmissionScheme) as any;
export const EventRecordSubmissionToAlias={
    "profileId": {
        "default": "i",
        "all": [
            "i"
        ]
    },
    "productId": {
        "default": "j",
        "all": [
            "j"
        ]
    },
    "productAltId": {
        "default": "ja",
        "all": [
            "ja"
        ]
    },
    "locationId": {
        "default": "l",
        "all": [
            "l"
        ]
    },
    "locationAltId": {
        "default": "la",
        "all": [
            "la"
        ]
    },
    "accountId": {
        "default": "a",
        "all": [
            "a"
        ]
    },
    "type": {
        "default": "t",
        "all": [
            "t"
        ]
    },
    "time": {
        "default": "o",
        "all": [
            "o"
        ]
    },
    "longText": {
        "default": "m",
        "all": [
            "m"
        ]
    },
    "text": {
        "default": "k",
        "all": [
            "k"
        ]
    },
    "tags": {
        "default": "g",
        "all": [
            "g"
        ]
    },
    "path": {
        "default": "p",
        "all": [
            "p"
        ]
    },
    "host": {
        "default": "h",
        "all": [
            "h"
        ]
    },
    "elem": {
        "default": "e",
        "all": [
            "e"
        ]
    },
    "classList": {
        "default": "c",
        "all": [
            "c"
        ]
    }
}
export const EventRecordSubmissionFromAlias={
    "i": {
        "default": "profileId",
        "all": [
            "profileId"
        ]
    },
    "j": {
        "default": "productId",
        "all": [
            "productId"
        ]
    },
    "ja": {
        "default": "productAltId",
        "all": [
            "productAltId"
        ]
    },
    "l": {
        "default": "locationId",
        "all": [
            "locationId"
        ]
    },
    "la": {
        "default": "locationAltId",
        "all": [
            "locationAltId"
        ]
    },
    "a": {
        "default": "accountId",
        "all": [
            "accountId"
        ]
    },
    "t": {
        "default": "type",
        "all": [
            "type"
        ]
    },
    "o": {
        "default": "time",
        "all": [
            "time"
        ]
    },
    "m": {
        "default": "longText",
        "all": [
            "longText"
        ]
    },
    "k": {
        "default": "text",
        "all": [
            "text"
        ]
    },
    "g": {
        "default": "tags",
        "all": [
            "tags"
        ]
    },
    "p": {
        "default": "path",
        "all": [
            "path"
        ]
    },
    "h": {
        "default": "host",
        "all": [
            "host"
        ]
    },
    "e": {
        "default": "elem",
        "all": [
            "elem"
        ]
    },
    "c": {
        "default": "classList",
        "all": [
            "classList"
        ]
    }
}
