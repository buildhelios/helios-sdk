...
export const getDefaultTimezone=()=>{
    ...
    const date = new Date();
    const dateString = new Intl.DateTimeFormat('en-US', { timeZoneName: 'short' }).format(date);
    const matches = dateString.match(/\b([A-Z]+)\b/);
    const browserTimezone = matches ? matches[0] : 'UTC';

    return timezones.find(({  abbr }) => abbr === browserTimezone) ?? gmt;
}

export const getTimezone=(abbr: string)=>{
    //logic
    return timezones.find((timezone)=>timezone.abbr===abbr) ?? getDefaultTimezone();
}
